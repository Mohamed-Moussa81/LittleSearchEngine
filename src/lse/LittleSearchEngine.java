...
public class LittleSearchEngine {
			...
			String word = file.next();
				...
				if (keyword != null) {
					if (words.containsKey(keyword)) {
						Occurrence name = words.get(keyword);
						name.frequency++;
						//words.put(keyword, name);
					} else {
	...
	public void mergeKeywords(HashMap<String, Occurrence> kws) {
		...
		for (String word : kws.keySet()) {
			// word is going to be each key in the hashmap
			if (keywordsIndex.containsKey(word)) {
				ArrayList<Occurrence> occurences = keywordsIndex.get(word);
				occurences.add(kws.get(word));
				// last item is just added and need to do binary search on that
				insertLastOccurrence(occurences);
				keywordsIndex.put(word, occurences);
			} else {
				Occurrence kwsOccurence = kws.get(word);
		...
		// takes the loaded keywords form one doc and puts it in main hashmap with
		// arraylist of occurence, this list
		// must be added to using insertLast occurence
	}
	...
	public String getKeyword(String word) {
		...
		/*
		 * word = word.replace(".",""); word = word.replace("?",""); word =
		 * word.replace(":",""); word = word.replace(";",""); word =
		 * word.replace("!",""); word.trim();
		 */
		if (word.equals("")) {
			return null;
		}
		/*
		 * while(word.charAt(word.length()-1) == ',') { word =
	...
	public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
		/** COMPLETE THIS METHOD **/
		// returned values are the mid point indices
		// add automatically shifts the values
		if(occs.size() ==1) {
			return null;
		}
		...
		occs.add(start, val);
		return midpoints;
		// pull out last value from arraylist and then do binaary serach to add in
		// decreasing order
		// following line is a placeholder to make the program compile
		// reverse binary serach pull out the last element first;
		// you should modify it as needed when you write your code
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		// case where one is empty and one isnt;
		String document = "";
		int kw1index = 0;
		int kw2index = 0;
