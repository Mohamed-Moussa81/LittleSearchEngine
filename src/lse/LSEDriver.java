...
public class LSEDriver {
	...
	public void insertLastOccurenceTester() throws FileNotFoundException {
		this.loadNoise();
		ArrayList<Occurrence> words = new ArrayList<>();
		Scanner sc = new Scanner(System.in);	
		System.out.print("Insert frequencies followed by done when done: ");
		while(sc.hasNextInt()) {
			Occurrence occ = new Occurrence("T",sc.nextInt());
			words.add(occ);
			String s = sc.next();
				if(s.equals("done")) {
					break;
					...
					occ = new Occurrence("T ",Integer.parseInt(s));
					words.add(occ);
				}
		}
		String done = sc.next();
		System.out.print("insert frequency to be added");
		int add = sc.nextInt();
		words.add(new Occurrence("T ",add));
		System.out.println(this.lse.insertLastOccurrence(words));
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		System.out.print("(a)test getWord\t(b)test loadKeyWords\t(c)test makeIndex\t(d)test top5search\t(e) test Insert: ");
		...
		switch (option) {
			...
			case "d" :
				...
				break;
			case "e":
				driver.insertLastOccurenceTester();
